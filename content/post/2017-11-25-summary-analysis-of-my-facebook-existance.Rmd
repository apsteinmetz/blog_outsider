...
for (pol in politicos) {
  ...
  if(nrow(sent_words)>0){
    ...
    pol_sent<-bind_rows(data_frame(Politician=str_to_title(pol),
                                   opinion=avg_score$opinion,
                                   post_count=avg_score$post_count,
                                   sent_words=list(sent_words$score)
                                   ),
    pol_sent)
...
Let's subject this to a test of signifcance. The "null hypothesis" we want to test is that the mean sentiment score expressed for "Hillary" is not statistically different from the "Trump" score.  
```{r}
