...
Let's subject this to a test of signifcance. The "null hypothesis" we want to test is that the mean sentiment score expressed for "Hillary" is not statistically different from the "Trump" score.  We duplicate some of the code run above, not for efficiency, but for clarity.
```{r}
trump_opinion<- cleaned_post_words %>% 
    filter(word == 'trump') %>%
    select(date) %>% 
    unique() %>% 
    inner_join(cleaned_post_words,by='date') %>%
...
hillary_opinion<- cleaned_post_words %>% 
    filter(word == 'hillary') %>%
    select(date) %>% 
    unique() %>% 
    inner_join(cleaned_post_words,by='date') %>%
