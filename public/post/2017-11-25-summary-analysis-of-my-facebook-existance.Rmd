...
Let's start out nice and easy. No big data machine learning or heavy stats.  This post will merely explore the depths of my soul through a meta-analysis of every one of my Facebook posts. Meta-navel gazing, if you will.
...
Note I use the 'tidyverse/dplyr' vernacular.  This is fast becoming a dialect of R.  I quite like it but its syntax is different than traditional R.  It produces sometimes slower, but much more readable, code. Basically, you "pipe" data tables through action verbs using the pipe operator ("%>%").  
...
```{r}
path='C:/Users/Arthur/Documents/R Projects/facebook/html/'
...
`</p><p><div class="meta">Thursday, November 16, 2017 at 1:17pm EST</div>`<br>
`<div class="comment">I’m sure you are all thinking “what does this mean for Al Franken?”</div>`<br>
`</p><p>`<br>
`<div class="meta">Thursday, November 16, 2017 at 10:44am EST</div>`<br>
`Art Steinmetz shared a link.`<br>
`</p><p>`<br>
...
for (n in 1:length(timeline_post_nodes)){
  if ( html_attr(timeline_post_nodes[n],"class")=="comment"){
    ...
    timeline_posts1<-timeline_posts1 %>% bind_rows(data_frame(date,post))
  }
}
```
...
nrow(my_post_words)
```
So we have over fifty thousand words.  A lot of them are going to be uninteresting.  Although, given that Facebook posts are an excercise in narcissim, you might say all of them are unintersting to anybody but me.
...
word_scores<-cleaned_post_words %>% 
  inner_join(get_sentiments('afinn'),by="word") %>% 
  group_by(word,score) %>% summarise(count=n())
...
```
Ohh, that is harsh. I am cringing right now.  Did I say those things?  Yes, well not often, at least.  As I mentioned above, the limitation of this analysis is that it lacks context.  For instance, did I call someone a slut?  I was briefly horrified when I saw that word.  Here is the word in context from 2014: "Less slut-shaming and more perp-jailing." 
...
What were my postive words?
```{r}
word_scores %>% arrange(desc(score)) %>% 
  ungroup() %>%
  .[1:10,] %>% 
  kable() %>% 
  kable_styling(bootstrap_options = "striped", full_width = FALSE, position='left')
...
```{r}
activity_times <- data_frame(date = raw_timeline %>% 
...
word_scores_by_month<-cleaned_post_words %>% 
  inner_join(get_sentiments('afinn'),by="word") %>% 
  select(date, word,score) %>% 
  mutate(yearmonth=as.yearmon(date)) %>% 
  group_by(yearmonth) %>% summarise(mood=sum(score))
...
"Hillary"" is the only first name in the list.  Using a woman's first name when
equally familiar men are referred to with their last name is often sexist.
In my defense, during the election it was important to distinguish between her and husband Bill so that's why I used "Hillary."  We are not on a first name basis. On the other hand, "Bill" is a common enough name (and noun) so it's likely that many posts using it don't refer to the politician.  Just to get an idea let's look at the words bracketing "bill":
...
for (pol in politicos) {
  ...
  if(nrow(sent_words)>0){
    avg_score<-summarise(sent_words,opinion=mean(score),post_count=n())
...
First off, Palin and McCain don't appear in this list beacuse I apparently didn't use any words from the "afinn" lexicon in my posts mentioning them.  Second, Romney is only mentioned in four posts so I don't think we have a valid sample size.

I don't recall being so positive about Trump.  Does it have to do with the positive sentiment for "trump" we saw in the word cloud above? No.  When we examine the "afinn" lexicon used in this section "trump" doesn't appear.

Taking what's left, let's view a chart.
```{r}
pol_sentiment %>% filter(Politician != "Romney") %>% 
...
I didn't really express strong opinions, it seems.  During the election things got pretty heated as you may recall. I have Facebook friends on all sides of the the political divides.  Out of respect, I tried very hard not to trash anybody's candidate and instead tried to accentuate the positive.  On that point, here's what I said in September, 2016:
```{r}
timeline_posts %>% 
  filter(date==as.POSIXct("2016-09-18 06:40:00")) %>% select(post) %>% 
  kable() %>% 
  kable_styling(bootstrap_options = "striped", full_width = FALSE, position='left')
...
It seems that adding friends is associated with deleted requests.  I'l surmise that when I show up in a new friend's network that will spark some friend requests from their network.  Some, maybe most, will be from people I don't actually know and I will reject.  There are spikes in rejections because I let them stack up before I notice them.
...
interp_moods<-data_frame(doy=unlist(lapply(levels(my_days_moods$doy),rep,7)),
                         mood_label=round(unlist(lapply(my_days_moods$mood,rep,7)),2),
                         mood=approx(x=1:7,my_days_moods$mood,n=7*7)$y)
...
source("C:/Users/Arthur/Documents/R Projects/facebook/g_speedometer.r")
