...
t_tidy <- function(df, rowlabel_col_name = "item"){
  ...
  df |> 
    ...
    pivot_wider(names_from = names(df)[1], values_from = value)
}

cars_df |> t_tidy("item") |> t_tidy()
cars |> t_tidy("item")
...
If you're a fan of the speedy `data.table` package, there is `transpose`  function that works just like the `t()` function so it's a simple matter to adapt our `t_df` function to use `data.table` instead of `t()`.  We'll call it `t_dt`.  Once again, we validate the function by transposing the `cars_df` data frame twice and comparing the result to the original data frame.
...
```
How much of this time difference is due to coercing the data frame to a tibble or data.table?  Let's test that out.
```{r}
microbenchmark(times = 100,
  t_df = data.frame(cars_df),
  t_tibble = tibble(cars_df),
  t_as_tibble = as_tibble(cars_df),
  t_dt = data.table(cars_df),
  t_as.dt = as.data.table(cars_df)
)
```
Let's benchmark a data frame with mixed data types, `iris`
```{r}
microbenchmark(times = 100,
  t_df = iris |> t_df("observation"),
  t_tidy = iris |> t_tidy("observation"),
  t_dt = iris |> t_dt("observation")
)
```
